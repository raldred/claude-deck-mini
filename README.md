...
Each of the 6 keys shows a running Claude Code session: a colored band plus a small glyph
for its state — red 🔒 (blocked on a permission prompt), amber ✓ (finished its turn), gold …
(idle, waiting on you), green (working), blue ⇊ (compacting context) — the repo and branch
it's working in, and how long since it last did anything. Sessions needing you bubble to the
top; if you have more than six, the last key pages through the rest. Glance at the deck: a
red key means an agent is blocked on you.
